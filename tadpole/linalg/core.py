...
###############################################################################
###                                                                         ###
###  Tensor linalg transformations                                          ###
###                                                                         ###
###############################################################################


# --- Linalg stack functor -------------------------------------------------- #
...
class LinalgStack:
   ...
   def __call__(self, xs, *args, **kwargs):
       ...
       l = IndexGen("l", tid.sizeof(*self._linds))
       r = IndexGen("r", tid.sizeof(*self._rinds)) 

       def reshape(x):
           x = tn.fuse(x, {self._linds: l, self._rinds: r})
           return tn.transpose(, l, r)

       xs  = tuple(map(reshape, xs))
       out = self._fun(xs, (l, r), **opts) 

       return tn.split(out, {l: self._linds, r: self._rinds})
...
@aligned
def lstack(xs, linds, rinds, **opts):

    stack = LinalgStack(lat.lstack, linds, rinds)
    return stack(xs, **ops)


@aligned
def rstack(xs, linds, rinds, **opts):

    stack = LinalgStack(lat.rstack, linds, rinds)
    return stack(xs, **ops)
...
def lstacked(fun):
    ...
    def wrap(xs, stackmap, **opts):

        xs = [tn.reindex(x, util.unpacked_dict(stackmap)) for x in xs]

        return fun(xs, linds=tuple(stackmap.values()), **opts) 
...
def rstacked(fun):
    ...
    def wrap(xs, stackmap, **opts):
        ...
        return fun(xs, rinds=tuple(stackmap.values()), **opts) 
