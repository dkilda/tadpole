...
def vjp_getitem(g, out, x, pos):

    return tn.SparseGrad(x.space(), pos, g.item())


def vjp_vjp_getitem(g, out, x, pos):

    return g[pos]


ad.makevjp(tn.getitem,  vjp_getitem)
ad.makevjp(vjp_getitem, vjp_vjp_getitem)
