...
class TestGradsElemwiseUnary:
   ...
   @pytest.mark.parametrize("indnames, shape, pos", [
      ...
      ["ijk", (2,3,4), (slice(None), 1,          slice(2,4))],
   ])
   ...
   @pytest.mark.parametrize("indnames, shape, pos, eleminds, elemaxes", [
      ...
      ["ijk", (2,3,4), (slice(None), 1,          slice(2,4)), "ijk", (0,1,2)],
      ...
      ["ijk", (2,3,4), (slice(None), 1,          slice(2,4)), "kij", (2,0,1)],
   ])
   ...
   @pytest.mark.parametrize("dtype, eleminds, elemaxes", [
      ...
      ["complex128", "kij", (2,0,1)],
   ])
   def test_ungetitem_by_inds(self, graddat, dtype, eleminds, elemaxes):
